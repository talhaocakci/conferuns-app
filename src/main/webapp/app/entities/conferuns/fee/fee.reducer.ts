...
import { IFee, defaultValue } from 'app/shared/model/conferuns/fee.model';

export const ACTION_TYPES = {
  SEARCH_FEES: 'fee/SEARCH_FEES',
...
export default (state: FeeState = initialState, action): FeeState => {
  switch (action.type) {
    case REQUEST(ACTION_TYPES.SEARCH_FEES):
    ...
    case FAILURE(ACTION_TYPES.SEARCH_FEES):
    ...
    case SUCCESS(ACTION_TYPES.SEARCH_FEES):
...
const apiSearchUrl = 'api/_search/fees';

// Actions

export const getSearchEntities: ICrudSearchAction<IFee> = (query, page, size, sort) => ({
  type: ACTION_TYPES.SEARCH_FEES,
  payload: axios.get<IFee>(`${apiSearchUrl}?query=${query}`)
});
