...
export const ACTION_TYPES = {
  SEARCH_CONFERENCES: 'conference/SEARCH_CONFERENCES',
...
export default (state: ConferenceState = initialState, action): ConferenceState => {
  switch (action.type) {
    case REQUEST(ACTION_TYPES.SEARCH_CONFERENCES):
    ...
    case FAILURE(ACTION_TYPES.SEARCH_CONFERENCES):
    ...
    case SUCCESS(ACTION_TYPES.SEARCH_CONFERENCES):
...
const apiSearchUrl = 'api/_search/conferences';

// Actions

export const getSearchEntities: ICrudSearchAction<IConference> = (query, page, size, sort) => ({
  type: ACTION_TYPES.SEARCH_CONFERENCES,
  payload: axios.get<IConference>(`${apiSearchUrl}?query=${query}`)
});
