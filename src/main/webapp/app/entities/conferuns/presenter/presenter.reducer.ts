...
export const ACTION_TYPES = {
  SEARCH_PRESENTERS: 'presenter/SEARCH_PRESENTERS',
...
export default (state: PresenterState = initialState, action): PresenterState => {
  switch (action.type) {
    case REQUEST(ACTION_TYPES.SEARCH_PRESENTERS):
    ...
    case FAILURE(ACTION_TYPES.SEARCH_PRESENTERS):
    ...
    case SUCCESS(ACTION_TYPES.SEARCH_PRESENTERS):
...
const apiSearchUrl = 'api/_search/presenters';

// Actions

export const getSearchEntities: ICrudSearchAction<IPresenter> = (query, page, size, sort) => ({
  type: ACTION_TYPES.SEARCH_PRESENTERS,
  payload: axios.get<IPresenter>(`${apiSearchUrl}?query=${query}`)
});
