...
export const ACTION_TYPES = {
  SEARCH_TALKS: 'talk/SEARCH_TALKS',
...
export default (state: TalkState = initialState, action): TalkState => {
  switch (action.type) {
    case REQUEST(ACTION_TYPES.SEARCH_TALKS):
    ...
    case FAILURE(ACTION_TYPES.SEARCH_TALKS):
    ...
    case SUCCESS(ACTION_TYPES.SEARCH_TALKS):
...
const apiSearchUrl = 'api/_search/talks';

// Actions

export const getSearchEntities: ICrudSearchAction<ITalk> = (query, page, size, sort) => ({
  type: ACTION_TYPES.SEARCH_TALKS,
  payload: axios.get<ITalk>(`${apiSearchUrl}?query=${query}`)
});
