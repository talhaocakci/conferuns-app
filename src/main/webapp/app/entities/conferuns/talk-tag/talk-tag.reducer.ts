...
export const ACTION_TYPES = {
  SEARCH_TALKTAGS: 'talkTag/SEARCH_TALKTAGS',
...
export default (state: TalkTagState = initialState, action): TalkTagState => {
  switch (action.type) {
    case REQUEST(ACTION_TYPES.SEARCH_TALKTAGS):
    ...
    case FAILURE(ACTION_TYPES.SEARCH_TALKTAGS):
    ...
    case SUCCESS(ACTION_TYPES.SEARCH_TALKTAGS):
...
const apiSearchUrl = 'api/_search/talk-tags';

// Actions

export const getSearchEntities: ICrudSearchAction<ITalkTag> = (query, page, size, sort) => ({
  type: ACTION_TYPES.SEARCH_TALKTAGS,
  payload: axios.get<ITalkTag>(`${apiSearchUrl}?query=${query}`)
});
